...
class SHA1 {
  ...
  static func hash(from string : String) -> String {
      let len = 64
      let originalMessage = [UInt8](string.utf8)
      var tmpMessage = [UInt8](string.utf8)
      ...
      tmpMessage += DigestHelpers.arrayOfBytes(originalMessage.count * 8, length: 64 / 8)
      ...
      for chunk in BytesSequence(chunkSize: chunkSizeBytes, data: tmpMessage) {
          ...
          for x in 0..<M.count {
              ...
              case 0...15:
                  ...
                  let le = DigestHelpers.toUInt32Array(chunk[start..<end])[0]
                  M[x] = le.bigEndian
                  break
              default:
                  M[x] = DigestHelpers.rotateLeft(M[x-3] ^ M[x-8] ^ M[x-14] ^ M[x-16], count: 1)
                  break
              }
          }
          ...
          for j in 0...79 {
              ...
              let temp = (DigestHelpers.rotateLeft(A, count:5) &+ f &+ E &+ M[j] &+ k) & 0xffffffff
              E = D
              D = C
              C = DigestHelpers.rotateLeft(B, count: 30)
              B = A
              A = temp
          }
